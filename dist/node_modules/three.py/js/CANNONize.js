            ...
            cannonData.shapes.forEach( function(e) {
                ...
                switch (e) {
                    ...
                    case 'Heightfield':
                        ...
                        var dx = node.scale.x * node.geometry.parameters.width / node.geometry.parameters.widthSegments;
                        var data = [];
                        for (var ix = 0; ix < gridX1; ++ix) {
                            data.push(new Float32Array(gridY1));
                            for (var iy = 0; iy < gridY1; ++iy) {
                                data[ix][iy] = node.scale.z * array[3 * (gridX1 * (gridY1 - iy - 1) + ix) + 2];
                            }
                        }
                        ...
                        position.x = -node.scale.x * node.geometry.parameters.width / 2;
                        position.y = -node.scale.y * node.geometry.parameters.height / 2;
                        break;
                    case 'Trimesh':
                        var vertices;
                        var indices;
